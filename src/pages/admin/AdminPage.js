...
function ReportCard({ proof, users }) {
  ...
  return React.createElement("div", {
    style: { background: "#FFF", borderRadius: 16, padding: 0, marginBottom: 14, border: "1px solid " + COLORS.brd, overflow: "hidden" }
  },
    // Header
    React.createElement("div", {
      onClick: function() { setExpanded(!expanded); },
      style: { padding: "16px 20px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", background: expanded ? COLORS.greenL : "#FFF" }
    },
    ...
    expanded && React.createElement("div", { style: { padding: "0 20px 20px", borderTop: "1px solid " + COLORS.brd } },
      ...
      React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 16, fontSize: 12 } },
        React.createElement("div", { style: { padding: 12, background: COLORS.bg, borderRadius: 10 } },
          React.createElement("div", { style: { color: COLORS.sub, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 } }, "ARRIVEE"),
          React.createElement("div", { style: { fontWeight: 700, fontSize: 18, color: COLORS.green } }, proof.checkIn ? proof.checkIn.time : "--"),
          React.createElement("div", { style: { fontSize: 10, color: COLORS.sub, marginTop: 2 } }, "\ud83d\udccd " + (proof.checkIn ? proof.checkIn.lat.toFixed(4) + ", " + proof.checkIn.lng.toFixed(4) : "--"))
        ),
        React.createElement("div", { style: { padding: 12, background: COLORS.bg, borderRadius: 10 } },
          React.createElement("div", { style: { color: COLORS.sub, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 } }, "DEPART"),
          React.createElement("div", { style: { fontWeight: 700, fontSize: 18, color: COLORS.green } }, proof.checkOut ? proof.checkOut.time : "--"),
          React.createElement("div", { style: { fontSize: 10, color: COLORS.sub, marginTop: 2 } }, "\ud83d\udccd " + (proof.checkOut ? proof.checkOut.lat.toFixed(4) + ", " + proof.checkOut.lng.toFixed(4) : "--"))
        )
      ),

      // Observations
      proof.observations && React.createElement("div", { style: { marginTop: 12, padding: 14, background: COLORS.bg, borderRadius: 10 } },
        React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: COLORS.txt, marginBottom: 8 } }, "Observations du compagnon"),
        React.createElement("div", { style: { display: "flex", gap: 12, marginBottom: 8, fontSize: 13 } },
          React.createElement("div", null, React.createElement("span", { style: { color: COLORS.sub } }, "Humeur : "), React.createElement("strong", null, proof.observations.mood)),
          React.createElement("div", null, React.createElement("span", { style: { color: COLORS.sub } }, "Forme : "), React.createElement("strong", null, proof.observations.physical))
        ),
        proof.observations.notes && React.createElement("div", { style: { fontSize: 13, color: COLORS.txt, lineHeight: 1.5, padding: 10, background: "#FFF", borderRadius: 8, border: "1px solid " + COLORS.brd } }, proof.observations.notes),
        proof.observations.alerts && proof.observations.alerts.length > 0 && React.createElement("div", { style: { marginTop: 8 } },
          proof.observations.alerts.map(function(a, i) { return React.createElement("div", { key: i, style: { padding: "6px 10px", background: COLORS.warnL, borderRadius: 8, fontSize: 12, color: COLORS.warn, fontWeight: 600, marginBottom: 4 } }, "\u26a0\ufe0f " + a); })
        )
      ),

      // Photos
      proof.photos && proof.photos.length > 0 && React.createElement("div", { style: { marginTop: 12 } },
        React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: COLORS.txt, marginBottom: 8 } }, "\ud83d\udcf8 Photos de la visite (" + proof.photos.length + ")"),
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 } },
          proof.photos.map(function(url, i) {
            return React.createElement("a", { key: i, href: url, target: "_blank", rel: "noopener noreferrer" },
              React.createElement("img", { src: url, alt: "Photo " + (i + 1), style: { width: "100%", aspectRatio: "1", objectFit: "cover", borderRadius: 8, border: "1px solid " + COLORS.brd } })
            );
          })
        )
      ),
      ...
      // Validation
      React.createElement("div", { style: { marginTop: 12, fontSize: 12, color: COLORS.sub } },
        "Validation : " + (proof.validationType === "signature" ? "\u270d\ufe0f Signature digitale" : "\ud83d\udc46 Bouton de confirmation"),
        proof.validationType === "signature" && proof.signatureData && React.createElement("div", { style: { marginTop: 6 } },
          React.createElement("img", { src: proof.signatureData, alt: "Signature", style: { height: 50, border: "1px solid " + COLORS.brd, borderRadius: 6, background: "#FFF" } })
        )
      )
    )
  );
...
export default function AdminPage() {
  ...
  var handleDeleteVisit = async function(id) {
    try { await FS.updateVisit(id, { status: "cancelled" }); flash("Visite annulee"); await load(); }
    catch (e) { flash(e.message, "error"); }
  };
  ...
  var handleExport = function() {
    ...
    visits.forEach(function(v) {
      ...
      csv += v.date + "," + (fam && fam.seniorName ? fam.seniorName : "") + "," + (comp && comp.name ? comp.name : "") + "," + v.duration + "h," + v.formula + "," + v.status + "," + (proof && proof.checkIn ? proof.checkIn.time : "") + "," + (proof && proof.checkOut ? proof.checkOut.time : "") + "," + (proof && proof.observations ? proof.observations.mood : "") + "," + (proof && proof.observations ? proof.observations.physical : "") + "," + (proof && proof.photos ? proof.photos.length : 0) + "\n";
    });
  ...
  // Filtered proofs for reporting
  var filteredProofs = proofs;
  if (reportFilter !== "all") {
    filteredProofs = proofs.filter(function(p) { return p.familyId === reportFilter || p.companionId === reportFilter; });
  }
    ...
    React.createElement("div", { style: { padding: 24, maxWidth: 960, margin: "0 auto" } },
      ...
      tab === "users" && React.createElement(React.Fragment, null,
        ...
        showCreate && React.createElement(Card, { style: { border: "2px solid " + COLORS.purpleM } },
          ...
          React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 } },
            ...
            React.createElement("div", null, React.createElement("label", { style: { fontSize: 11, fontWeight: 600, color: COLORS.sub } }, "Email"), React.createElement("input", { value: nu.email, onChange: function(e) { setNu({ ...nu, email: e.target.value }); }, style: inputStyle, placeholder: "marie@vivalien.fr" })),
            ...
            nu.role === "family" && React.createElement(React.Fragment, null,
              ...
              React.createElement("div", null, React.createElement("label", { style: { fontSize: 11, fontWeight: 600, color: COLORS.sub } }, "Adresse senior"), React.createElement("input", { value: nu.seniorAddress, onChange: function(e) { setNu({ ...nu, seniorAddress: e.target.value }); }, style: inputStyle })),
      ...
      tab === "visits" && React.createElement(React.Fragment, null,
        ...
        showCreateVisit && React.createElement(Card, { style: { border: "2px solid " + COLORS.purpleM } },
          ...
          React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 } },
            React.createElement("div", null, React.createElement("label", { style: { fontSize: 11, fontWeight: 600, color: COLORS.sub } }, "Famille / Senior"), React.createElement("select", { value: nv.familyId, onChange: function(e) { setNv({ ...nv, familyId: e.target.value }); }, style: { ...inputStyle, cursor: "pointer" } }, React.createElement("option", { value: "" }, "-- Choisir --"), families.map(function(f) { return React.createElement("option", { key: f.id, value: f.id }, (f.seniorName || "") + " (" + f.name + ")"); }))),
            ...
            React.createElement("div", null, React.createElement("label", { style: { fontSize: 11, fontWeight: 600, color: COLORS.sub } }, "Duree"), React.createElement("select", { value: nv.duration, onChange: function(e) { setNv({ ...nv, duration: e.target.value }); }, style: { ...inputStyle, cursor: "pointer" } }, React.createElement("option", { value: 3 }, "3 heures"), React.createElement("option", { value: 4 }, "4 heures"))),
            React.createElement("div", null, React.createElement("label", { style: { fontSize: 11, fontWeight: 600, color: COLORS.sub } }, "Formule"), React.createElement("select", { value: nv.formula, onChange: function(e) { setNv({ ...nv, formula: e.target.value }); }, style: { ...inputStyle, cursor: "pointer" } }, FORMULAS.map(function(f) { return React.createElement("option", { key: f.id, value: f.name }, f.name + " - " + f.visit3h + "\u20ac"); })))
          ),
          React.createElement("div", { style: { display: "flex", gap: 8, marginTop: 14, justifyContent: "flex-end" } },
      ...
      tab === "reports" && React.createElement(React.Fragment, null,
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
          ...
          React.createElement("select", { value: reportFilter, onChange: function(e) { setReportFilter(e.target.value); }, style: { ...inputStyle, width: "auto", minWidth: 200, marginTop: 0 } },
            React.createElement("option", { value: "all" }, "Tous les comptes-rendus"),
            React.createElement("optgroup", { label: "Par senior" },
              families.map(function(f) { return React.createElement("option", { key: f.id, value: f.id }, f.seniorName || f.name); })
            ),
            React.createElement("optgroup", { label: "Par compagnon" },
        ...
        filteredProofs.length === 0
          ? React.createElement(Card, { style: { textAlign: "center", padding: 40, color: COLORS.sub } }, "Aucun compte-rendu")
          : filteredProofs.map(function(p) {
            return React.createElement(ReportCard, { key: p.id, proof: p, users: users });
          })
      ),
